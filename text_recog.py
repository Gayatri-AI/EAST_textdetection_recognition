...
# Access to webcam for video capture
...
while cv2.waitKey(1) < 0:
	...
	for (startX, startY, endX, endY) in boxes:
		...
		text_recognised = pytesseract.image_to_string(text)
		cv2.rectangle(orig, (startX, startY), (endX, endY), (0, 255, 0), 2)
		# putting text in image
		orig = cv2.putText(orig, text_recognised, (endX, endY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0,0), 1, cv2.LINE_AA)

	# Displaying detected image
	orig = cv2.resize(orig, (960, 640), interpolation=cv2.INTER_AREA)
	cv2.imshow("Text Recognise", orig)
